"""
External Adapter — MV Soul Adapter (Placeholder)

Este módulo contém o adaptador para o MV Soul (ERP Hospitalar).
...
class MVSoulAdapter(HospitalAdapterBase):
    """Adaptador para o MV Soul (ERP Hospitalar).
    ...
    async def get_hospital(self, hospital_id: str) -> HospitalData:
        """Obter dados de um hospital via MV Soul."""
        raise NotImplementedError("MVSoulAdapter não implementado")

    async def list_hospitals(self) -> list[HospitalData]:
        """Listar hospitais via MV Soul."""
        raise NotImplementedError("MVSoulAdapter não implementado")

    async def get_hospital_schedule(
        self, hospital_id: str, schedule_date: "date"
    ) -> ScheduleData:
        """Obter agenda via MV Soul."""
        raise NotImplementedError("MVSoulAdapter não implementado")
