...
logger = get_logger("use_case.period.close")


class ClosePeriod(BasePeriodUseCase):
    ...
    def execute(self, id: int = None, **kwargs) -> Success:
        ...
        error = self._state_machine.validate_transition(
            PeriodStatus(period.status), PeriodStatus.CLOSED
        )
        if error:
        ...
        self._queue_event(
            DomainEventName.PERIOD_CLOSED_V1,
            {"id": updated.id, "year": updated.year, "month": updated.month},
        )

        logger.info("period.closed.v1", extra={"period_id": updated.id})
